...
export class PlayOverlayRenderer {
  private overlayCache: OverlayCache | null = null;

  constructor(private readonly isBrowser: boolean) {}

  drawOverlay(frame: HandTrackingFrame, canvas: HTMLCanvasElement, video: HTMLVideoElement): void {
    if (!this.isBrowser || this.isDocumentHidden()) {
      return;
    }
  ...
  attachAspectListener(video: HTMLVideoElement, setAspect: (aspect: number) => void): void {
    const updateAspect = () => {
  ...
  primeVideoElement(video: HTMLVideoElement): void {
    video.autoplay = true;
    video.muted = true;
  ...
  private drawThereminOverlay(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ...
    const drawKnob = (x: number, y: number, radius: number, angleRadians: number): void => {
    ...
    const metalStroke = (x: number): CanvasGradient => {
      const gradient = ctx.createLinearGradient(x - antennaWidth, 0, x + antennaWidth, 0);
      gradient.addColorStop(0, 'rgba(226,232,240,0.82)');
      gradient.addColorStop(0.5, 'rgba(148,163,184,0.95)');
      gradient.addColorStop(1, 'rgba(248,250,252,0.82)');
      return gradient;
    };
