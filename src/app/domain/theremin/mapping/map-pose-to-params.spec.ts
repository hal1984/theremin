...
import { mapPoseToThereminParams, smoothParams, type ThereminMappingConfig } from './map-pose-to-params';
...
const makeFrame = (params: { right?: Partial<{ x: number; y: number; z: number }>; left?: Partial<{ x: number; y: number; z: number }> } = {}): HandTrackingFrame => {
  const hands = [];
  if (params.right) {
    hands.push({
...
const baseConfig: ThereminMappingConfig = {
  minHz: 80,
  maxHz: 800,
  volumeCurve: 'linear',
  quantize: false,
...
describe('mapPoseToThereminParams', () => {
  it('keeps previous pitch and returns 0 gain when no hands are present', () => {
    ...
    const next = mapPoseToThereminParams(frame, baseConfig, previous);
  ...
  it('clamps pitch and gain into valid ranges', () => {
    ...
    const config: ThereminMappingConfig = { ...baseConfig, minHz: 120, maxHz: 240 };
    const next = mapPoseToThereminParams(frame, config, previous);
  ...
  it('quantize=true snaps the computed pitch to the nearest semitone', () => {
    ...
    const unquantized = mapPoseToThereminParams(frame, { ...baseConfig, quantize: false }, previous);
    const quantized = mapPoseToThereminParams(frame, { ...baseConfig, quantize: true }, previous);

    const midi = 69 + 12 * Math.log2(unquantized.pitchHz / 440);
    const expected = 440 * Math.pow(2, (Math.round(midi) - 69) / 12);
...
describe('smoothParams', () => {
  it('returns next when smoothingMs<=0 or deltaMs<=0', () => {
    ...
    const next: ThereminParams = { pitchHz: 200, gain: 0.9 };
  ...
  it('moves monotonically from previous toward next', () => {
    const prev: ThereminParams = { pitchHz: 100, gain: 0.1 };
    const next: ThereminParams = { pitchHz: 200, gain: 0.9 };
    const smoothed = smoothParams(prev, next, 40, 16);
